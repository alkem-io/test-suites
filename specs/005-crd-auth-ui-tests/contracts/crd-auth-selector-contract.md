...
>
> **Update 2026-06-10:** Rows 1 and 2 re-verified against the running CRD build
> while fixing two suite failures. Both earlier conclusions were superseded —
> see the revised findings below.

| # | Screen / element | Finding | Proposed follow-up | Status in suite |
|---|---|---|---|---|
| 1 | Restricted-access → sign-in | **Superseded (2026-06-10).** `returnUrl` **IS** honored: after signing in from the restricted prompt the now-authenticated user is returned to the originally requested page (`/admin/spaces`), not `/home`. The earlier "redirects to `/home`" observation was incorrect/transient. Because the regular user lacks access, that page renders the authenticated "Access Restricted" variant ("Go to Home" button). No regression — behavior matches the pre-migration return-URL preservation (FR-005). | None — closed; behavior is correct. | Resolved: test renamed `…returns to restricted page authenticated`, asserts signed-in header avatar + restricted page in `authentication-restricted-access.spec.ts` |
| 2 | Password recovery | **Updated (2026-06-10).** Local Kratos sends a recovery **link** (`self-service/recovery?flow=…&token=…`), **not** a 6-digit code — so the `isLocalEnv` code branch was invalid for local, not merely blocked by the `getRecoveryCode` bug. The pre-existing `getRecoveryCode` TDZ bug (`lib/src/utils/emails.ts:53`) is now **fixed**. The suite uses the link flow for both local and remote, dropping the code branch entirely. (b) CRD set-password still rejects reusing the current password, so the old reuse-same-password idempotency was replaced. | (a) Closed — bug fixed and code branch removed; (b) handled in-test | Resolved: link flow for all envs; sets temp password then restores shared default in a `finally` |
