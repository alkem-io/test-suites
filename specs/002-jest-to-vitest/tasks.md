...
- [x] T004 Update `server-api/tsconfig.json`: change `"types": ["node", "jest"]` to `"types": ["node"]` and add `/// <reference types="vitest/globals" />` in `vitest-extend.d.ts` (custom `typeRoots` prevented direct `types` array resolution — see research.md §7). This enables TypeScript to recognize Vitest global types (`describe`, `it`, `expect`, `beforeAll`, etc.)
- [x] T005 Create `server-api/vitest.config.ts` with the complete Vitest configuration. Includes: `vite-tsconfig-paths` plugin, `environment: 'node'`, `globals: true`, `testTimeout: 60_000` (per-test), `hookTimeout: 120_000` (per-hook), `globalSetup: './src/globalTestsSetup.ts'`, `setupFiles: ['./src/setupTests.ts', './src/vitest.setup.ts']`, `reporters: ['default', 'html']`, `outputFile: { html: './html-report/report_${timestamp}.html' }` (timestamped), a `project()` helper function (sets `globalSetup: []` per-project to prevent duplicate user registration), and the complete `projects` array with 27 named domain projects plus the nightly composite (orphan `roleset-parallel` excluded). See `specs/002-jest-to-vitest/contracts/vitest-config-contract.md` for the full project definitions
