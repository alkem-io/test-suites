...
test.describe('US3 — anonymous Dutch offer', () => {
  ...
  test('US3-SC005 — anonymous pre-answer display is the platform default (English), NOT auto-switched to Dutch', async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: testInfo.outputPath('us3-sc005-preanswer-display.png'), fullPage: false });
    expect(
      lang,
  ...
  test('US3-AS2b — language banner not shown until consent resolved', async ({ page }, testInfo) => {
    await landAnonymously(page);
    ...
    // Language banner must NOT appear while consent is unanswered, even though the
    // browser is nl-NL — and must stay away, not merely be late.
    await expectNoLanguageOffer(page, 'FR-013b-ii: no language offer before cookie consent is answered');

    await page.screenshot({
      path: testInfo.outputPath('us3-as2b-consent-unanswered-no-language-banner.png'),
      fullPage: false,
    });
  });
  ...
  test('US3-AS1 — Dutch banner appears after consent, written in Dutch', async ({ page }, testInfo) => {
    await landAnonymously(page);
    ...
    await page.screenshot({ path: testInfo.outputPath('us3-as1-dutch-banner.png'), fullPage: false });
  });
  ...
  test('US3-AS2 — accept renders Dutch for the session; nothing stored; reload re-offers', async ({ page }, testInfo) => {
    await landAnonymously(page);
    await consentAcceptCookies(page);
    ...
    await page.screenshot({ path: testInfo.outputPath('us3-as2-accept-session-only.png'), fullPage: false });
  });
  ...
  test('US3-AS2a — accept writes zero language keys to browser storage (SC-001c)', async ({ page }, testInfo) => {
    await landAnonymously(page);
    await consentAcceptCookies(page);
    ...
    await page.screenshot({ path: testInfo.outputPath('us3-as2a-accept-zero-storage.png'), fullPage: false });
  });
  ...
  test('US3-AS3 — decline stays English; nothing stored; not re-shown this session', async ({ page }, testInfo) => {
    await landAnonymously(page);
    await consentAcceptCookies(page);
    ...
    // Not re-shown within the same session (the answer is latched in memory). The
    // anonymous surface offers no SPA-internal navigation (see us3b), so the
    // in-session re-check is: keep the same document, re-mount the layout by
    // resizing, and assert the banner never comes back.
    await page.setViewportSize({ width: 1024, height: 768 });
    await expect(footerLanguageEnglish(page)).toBeVisible();
    await expectNoLanguageOffer(page, 'the declined offer must not return within the same session');

    await page.screenshot({ path: testInfo.outputPath('us3-as3-decline-session-only.png'), fullPage: false });
  });
});
