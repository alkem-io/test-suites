...
###### 1.1.A Whiteboard Template CRUD (Space Settings Library)
**Steps:**
1. Sign in as the seeded Space Admin and navigate to the target space (e.g., `seed-minimal-537777`). Open the `Settings` tab and then `Templates` to reach the space-level library.
2. Inside the `Whiteboard Templates` section, click `Create new`, fill the metadata form (title, markdown description using headings, and at least two tags), then launch the Excalidraw editor via `Edit`.
3. In the editor, explicitly select the Text tool (`Text — T or Enter`), click once on the canvas, type the desired copy (e.g., `Whiteboard Template Generated by Playwright`), press `Escape` to commit the text block, and click `Save` to persist and exit the editor.
4. Submit the template with `Create` and verify the toast `Template saved successfully`, the new card entry (tags + thumbnail), and the preview dialog content (Title, Description, Tags, canvas rendering).
5. Re-open the template card, use `Edit`, and update the metadata: append a unique suffix to the title/description, add a new tag, re-open the whiteboard editor, double-click the existing text, replace it with the updated string, press `Escape`, and `Save` the canvas before clicking `Update`.
6. After updating, confirm the preview dialog shows the refreshed metadata, tag list (including the new tag), and the updated canvas text. Then start the delete flow from `Edit → Delete`, assert the warning copy `Are you sure you want to delete the Template '<updated name>'?`, confirm deletion, and verify the list count returns to `Whiteboard Templates (0)`.

**Expected Results:**
- Canvas text updates only persist when `Escape` is pressed prior to `Save`; skipping this leaves the modal in edit mode, so the test must mimic the real workflow.
- Toast notification `Template saved successfully` appears after both create and update operations, and the preview dialog reflects the latest metadata, tags, and whiteboard snapshot.
- Delete confirmation modal blocks until the warning is acknowledged; upon success the template card disappears and section counters decrement without page reloads.
