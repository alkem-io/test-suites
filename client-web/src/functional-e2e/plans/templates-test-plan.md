...
3. In the editor, explicitly select the Text tool (`Text — T or 8`), click once on the canvas, type the desired copy (e.g., `Whiteboard Template Generated by Playwright`), press `Escape` to commit the text block, and click `Save` to persist and exit the editor.
