...
- **Subspace Visibility**: Subspaces are only visible to **direct members of their parent space**. This means:
  ...
  - Level 1 members can see Level 2 subspaces (but NOT Level 0 root spaces unless they're also members)
...
The test scenarios are organized by space level to facilitate focused testing and maintenance. Each level includes tests for discovery, application submission, notifications, and application review workflows.
...
#### 1.1 View Private Space as Non-Member
...
- Level 1 (Subspace) and Level 2 (Subsubspace) are NOT visible to non-members
- Level 0 shows "Apply" or "Join" button available
- Non-member cannot directly access space content beyond the landing page

#### 1.2 View Level 0 as Member

**Steps:**

1. Sign in as `${TestUser.SPACE_MEMBER}@alkem.io` (a user who is a member of Level 0 only)
2. Navigate to spaces listing
3. Navigate into the Level 0 Space
4. Verify Level 1 subspaces are visible in the Subspaces section
5. Verify Level 2 subsubspaces are NOT visible

**Expected Results:**

- Space (Level 0) is the root and visible on the home page
- Subspace (Level 1) appears nested under Space in the Subspaces section (visible because user is a direct member of Level 0)
- Subsubspace (Level 2) is NOT visible anywhere (user must be a member of Level 1 to see Level 2, not just Level 0)
- Level 0 member can only see their own space (Level 0) and its direct children (Level 1)
- Tree structure clearly indicates hierarchy through breadcrumbs and URL path

### 2. Level 0 Application Submission
...
#### 2.2 Prevent Duplicate Applications to Level 0
...
### 3. Level 0 Admin Notifications

#### 3.1 Receive Notification for Level 0 Application
...
#### 3.2 Navigate from Notification to Level 0 Application Management

**Steps:**

1. Sign in as `${TestUser.SPACE_ADMIN}@alkem.io`
2. Click bell icon to view notifications
3. Click on a Level 0 application notification

**Expected Results:**

- User is navigated to the Level 0 space settings page
...
### 4. Level 0 Application Review

#### 4.1 Level 0 Admin Access to Applications
...
- Applications list is visible and manageable

#### 4.2 View Level 0 Application Questionnaire
...
#### 4.3 Approve Level 0 Application
...
#### 4.4 Verify Approved User Gains Access to Level 0
...
#### 4.5 Reject Level 0 Application
...
#### 4.6 Verify Rejected User Does Not Gain Access to Level 0

**Steps:**

1. Admin rejects application for Level 0 Space
2. Sign out and sign in as `${TestUser.NON_SPACE_MEMBER}@alkem.io`
3. Navigate to Level 0 Space
...
#### 4.7 Delete Level 0 Application
...
#### 4.8 Multiple Applications Display for Level 0
...
#### 4.9 Mixed Actions on Multiple Level 0 Applications
...
### 5. Subspace Discovery and Privacy Indicators

#### 5.1 View Level 1 as Level 0 Member

**Steps:**

1. Sign in as `${TestUser.SPACE_MEMBER}@alkem.io` (a user who is a member of Level 0 only)
2. Navigate into the Level 0 Space
3. Locate Level 1 Subspace in the Subspaces section
4. Verify lock icon and Apply button
5. Verify Level 2 subsubspaces are NOT visible anywhere

**Expected Results:**

- Subspace (Level 1) is visible in the Subspaces section (because user is a direct member of Level 0)
- Lock icon indicates user doesn't have access yet
- Apply button is available (membership is not inherited from parent)
- Level 2 subsubspaces are NOT visible (Level 0 member can only see Level 1, not Level 2)

#### 5.2 View Level 1 as Member with Level 2 Visible

**Steps:**

1. Sign in as `${TestUser.SUBSPACE_MEMBER}@alkem.io` (a user who is a member of Level 1 only)
2. Navigate to Level 0 Space (visible because user needs to traverse through it)
3. Navigate to Level 1 Subspace
4. Verify Level 2 subsubspace is visible with lock icon in the Subspaces section
5. Click into Level 2 subsubspace and verify Apply button is present

**Expected Results:**

- Subsubspace (Level 2) appears in the Subspaces section under Level 1 (visible because user is a direct member of Level 1)
- Lock icon is displayed on the Level 2 subsubspace card (indicating user doesn't have access yet)
- Apply button is available when clicking into Level 2 subsubspace
- User can see Level 2 but cannot access content without applying and being approved
- Level 1 member can only see their own space (Level 1) and its direct children (Level 2), plus parent spaces needed for navigation

### 6. Level 1 Application Submission

#### 6.1 Apply to Level 1 Subspace

**Steps:**

1. Sign in as `${TestUser.NON_SPACE_MEMBER}@alkem.io`
2. First, become a member of Level 0 Space (apply and get approved, or assume already approved)
3. Navigate into the Level 0 Space to see the Subspaces section
4. Navigate to the Level 1 Subspace (now visible since user is a member of parent space)
5. Click the "Apply" button (note: being a member of parent does NOT grant automatic membership)
6. Fill in questionnaire with test answers:
...
- User can see Level 1 Subspace in the Subspaces section (because they are a member of Level 0)
- Apply button is visible on Level 1 subspace (membership is not inherited from parent)
- Questionnaire appears specific to the subspace
- Application is successfully submitted
- Confirmation message is displayed

### 7. Level 1 Admin Notifications

#### 7.1 Level 0 Admin Receives Notification for Level 1 Application
...
#### 7.2 Level 1 Admin Receives Notification for Level 1 Application
...
#### 7.3 No Notification for Level 0 Application to Level 1 Admin
...
### 8. Level 1 Application Review

#### 8.1 Level 0 Admin Access to Level 1 Applications
...
4. Click Community tab in subnavigation
5. Locate Applications section
6. Verify application management buttons are visible

**Expected Results:**

- Level 0 admin can access applications for Level 1 Subspace
- Applications section is visible
- Application management buttons (View, Approve, Reject, Delete) are present and enabled
- Level 0 admin has full access due to inherited permissions

#### 8.2 Level 1 Admin Access to Level 1 Applications

**Steps:**

1. Sign in as `${TestUser.SUBSPACE_ADMIN}@alkem.io`
2. Navigate to Level 1 Subspace settings → Community tab
3. Locate Applications section
4. Verify application management buttons are visible
...
#### 8.3 Level 1 Admin Cannot Access Level 0 Applications

**Steps:**

1. Sign in as `${TestUser.SUBSPACE_ADMIN}@alkem.io`
2. Attempt to navigate to Level 0 Space settings
3. Try to access Community tab and Applications

**Expected Results:**

- Level 1 admin cannot access Level 0 Space application management (or settings are restricted)
- Access denied or limited view
- No application management buttons visible for Level 0

---

## Level 2 (Subsubspace) Test Suite

**Seed:** `./client-web/src/functional-e2e/seed-applications.spec.ts`

### 9. Subsubspace Discovery and Privacy Indicators

#### 9.1 View Level 2 as Level 1 Member

**Steps:**

1. Sign in as `${TestUser.SUBSPACE_MEMBER}@alkem.io` (a user who is a member of Level 1 only)
2. Navigate to Level 0 Space → Level 1 Subspace
3. Locate Level 2 Subsubspace in the Subspaces section
4. Verify lock icon and Apply button

**Expected Results:**

- Subsubspace (Level 2) is visible in the Subspaces section (because user is a direct member of Level 1)
- Lock icon indicates user doesn't have access yet
- Apply button is available (membership is not inherited from parent)
- Level 1 member can only see Level 2 subspaces, not any deeper levels (if they existed)

#### 9.2 View Level 2 as Member with Full Access

**Steps:**

1. Sign in as `${TestUser.SUBSUBSPACE_MEMBER}@alkem.io` (a user who is a member of Level 2 only)
2. Navigate to Level 0 Space → Level 1 Subspace → Level 2 Subsubspace
3. Verify full access to Level 2 content

**Expected Results:**

- Subsubspace (Level 2) appears without lock icon (user has access)
- NO Apply button is shown (user is already a member)
- User can access all Level 2 content
- URL path shows full hierarchy: `/space-nameid/subspace-nameid/subsubspace-nameid`
- Note: Level 2 member may need appropriate parent memberships to navigate through Level 0 and Level 1 to reach Level 2

### 10. Level 2 Application Submission

#### 10.1 Apply to Level 2 Subsubspace

**Steps:**

1. Sign in as `${TestUser.NON_SPACE_MEMBER}@alkem.io`
2. First, become a member of Level 0 Space and Level 1 Subspace (apply and get approved for each, or assume already approved)
3. Navigate through Level 0 → Level 1 to access Level 2 Subsubspace
4. Navigate to the Level 2 Subsubspace (now visible since user is a member of parent subspace)
5. Click the "Apply" button (note: being a member of parent subspace does NOT grant automatic membership)
6. Fill in questionnaire with test answers:
...
- User can see Level 2 Subsubspace (because they are a member of Level 1)
- Apply button is visible on Level 2 subsubspace (membership is not inherited from parent)
- Questionnaire opens correctly
- Application is successfully submitted
- User receives confirmation

### 11. Level 2 Admin Notifications

#### 11.1 Level 0 Admin Receives Notification for Level 2 Application
...
#### 11.2 Level 1 Admin Receives Notification for Level 2 Application
...
#### 11.3 Level 2 Admin Receives Notification for Level 2 Application
...
#### 11.4 No Notification for Level 0 or Level 1 Applications to Level 2 Admin
...
### 12. Level 2 Application Review

#### 12.1 Level 0 Admin Access to Level 2 Applications
...
#### 12.2 Level 1 Admin Access to Level 2 Applications

**Steps:**

1. Sign in as `${TestUser.SUBSPACE_ADMIN}@alkem.io`
2. Navigate to Level 2 Subsubspace settings → Community tab
3. Locate Applications section
4. Verify application management buttons are visible
...
- Applications section is visible
- Application management buttons (View, Approve, Reject, Delete) are present and enabled
- Level 1 admin has access due to inherited permissions

#### 12.3 Level 2 Admin Access to Level 2 Applications Only

**Steps:**

1. Sign in as `${TestUser.SUBSUBSPACE_ADMIN}@alkem.io`
2. Navigate to Level 2 Subsubspace settings → Community tab
3. Verify access to Level 2 applications and buttons
4. Attempt to navigate to Level 0 and Level 1 settings

**Expected Results:**

- Level 2 admin can access only Level 2 Subsubspace applications
- Application management buttons (View, Approve, Reject, Delete) are visible for Level 2
- Level 2 admin cannot access Level 0 Space application management
- Level 2 admin cannot access Level 1 Subspace application management

---

## Cross-Level Test Suite

**Seed:** `./client-web/src/functional-e2e/seed-applications.spec.ts`

### 13. Applicant Notifications

#### 13.1 Receive Application Confirmation Notification

**Steps:**

1. Sign in as `${TestUser.NON_SPACE_MEMBER}@alkem.io`
2. Submit application to any level space
3. Click the bell icon in the upper right corner
...
#### 13.2 Receive Application Approval Notification

**Steps:**

1. Sign in as `${TestUser.NON_SPACE_MEMBER}@alkem.io`
2. Wait for admin to approve application
...
- Message clearly states: "Your application to [Space Name] has been approved"
- Notification includes reviewer action timestamp
- Clicking notification navigates to relevant space or shows details

#### 13.3 Receive Application Rejection Notification

**Steps:**

1. Sign in as `${TestUser.NON_SPACE_MEMBER}@alkem.io`
2. Wait for admin to reject application
3. Click bell icon
...
#### 14.1 Submit Application with Empty Questionnaire
...
#### 14.2 Navigate Away During Application
...
#### 14.3 Concurrent Application Review
...
#### 14.4 Network Error During Application Submission
...
#### 14.5 Permission Changes During Session
...
#### 14.6 Verify Deleted Application Cannot Be Recovered
...
#### 15.2 Notification Badge Updates
...
| `${TestUser.SPACE_MEMBER}@alkem.io`       | Level 0 Member | Level 0 only    |
| `${TestUser.SUBSPACE_MEMBER}@alkem.io`    | Level 1 Member | Level 1 only    |
| `${TestUser.SUBSUBSPACE_MEMBER}@alkem.io` | Level 2 Member | Level 2 only    |
...
4. **Test Implementation Optimization**: When implementing automated tests, sign in users once in `test.beforeAll()` or `test.beforeEach()` hooks instead of in each individual test to improve performance and reduce code duplication. Store the authenticated page/context for reuse across tests within the same describe block.

5. **Notification Timing**: Some tests may require brief waits for notifications to appear (real-time vs polling)

6. **Inherited Permissions Testing**: Pay special attention to notification and access control scenarios which validate the permission inheritance model

7. **Cross-Browser Testing**: Run critical paths on Chrome, Firefox, and Safari to ensure consistency

8. **Accessibility**: Validate that keyboard navigation works for all application workflows (Tab, Enter, Escape)

9. **Frontend Caching**: The Settings → Community page may cache data. If expected information (applications, notifications, member lists) is missing or not updated, **refresh the page first** before reporting a bug. This is especially important after:
...
10. **Using baseScenario for Navigation**: The `baseScenario` variable (of type `OrganizationWithSpaceModel`) contains the space tree created for the test run. When implementing automated tests, use this structure to navigate to specific spaces programmatically:
...
// Note: Subspaces are only visible after joining the parent space
const subspaceDisplayName = baseScenario.subspace.about.profile.displayName;
...
**Note**: Space cards are rendered as links containing the display name (h2 heading), avatar, banner image, and a lock icon (`LockOutlinedIcon`) for private spaces.
