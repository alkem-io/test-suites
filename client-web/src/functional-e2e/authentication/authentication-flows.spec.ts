/**
 * @deprecated This file has been refactored and split into independent test files.
 *
 * Tests have been moved to:
 * - authentication-page-verification.spec.ts (page element verification tests)
 * - authentication-login.spec.ts (login and logout flows)
 * - authentication-registration.spec.ts (registration with email verification)
 * - authentication-password-recovery.spec.ts (password recovery flow)
 *
 * The original file used serial mode which caused cascading test skips when one test failed.
 * The new files run tests independently for better stability and isolation.
 *
 * This file is preserved for reference only. All tests are skipped.
 * Remove this file once the new test structure is validated.
 */
...
test.describe.skip(
  'DEPRECATED - Authentication Flows (see new split files)',
  () => {
