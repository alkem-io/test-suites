...
function longText(label: string, sentences: number): string {
  ...
  return Array.from({ length: sentences }, (_, i) => `${base}, sentence ${i + 1} of this fixture card.`).join(' ');
}
...
const CARDS: CardDef[] = [
  { label: 'CardAlpha', what: longText('CardAlpha', 4), why: longText('CardAlpha why', 3), who: longText('CardAlpha who', 3) },
  { label: 'CardBeta', what: longText('CardBeta', 4), why: longText('CardBeta why', 3), who: longText('CardBeta who', 3) },
  { label: 'CardGamma', what: longText('CardGamma', 4), why: longText('CardGamma why', 3), who: longText('CardGamma who', 3) },
  { label: 'CardDelta', what: longText('CardDelta', 4), why: longText('CardDelta why', 3), who: longText('CardDelta who', 3) },
  { label: 'CardEta', what: longText('CardEta', 4), why: longText('CardEta why', 3), who: longText('CardEta who', 3) },
];
...
async function createSubspace(spaceId: string, label: string, what: string, why: string, who: string): Promise<string> {
  const data = await rawGql<{ createSubspace: { id: string } }>(
    ...
    }`,
    {
      subspaceData: {
        ...
        about: {
          profileData: { displayName: label, description: what, tagline: `${label} fixture card` },
          why,
          who,
        },
...
test.describe('Read expanded cards on a phone (US4)', { tag: '@forge-acceptance' }, () => {
  ...
  test.beforeAll(async () => {
    ...
    const space = await rawGql<{
      ...
      }`,
      {
        spaceData: {
          accountID,
          nameID: spaceNameId,
          about: { profileData: { displayName: `US4 Narrow Layout ${runSuffix}` } },
          collaborationData: { calloutsSetData: {} },
          settings: { privacy: { mode: 'PUBLIC' } },
        },
      },
    ...
    for (const c of CARDS) {
      subspaceIds.push(await createSubspace(spaceId, c.label, c.what, c.why, c.who));
    }
  ...
  async function gotoFixtureSpace(page: Page) {
    ...
    await expect(page.getByText(POST_TITLE, { exact: true }).first()).toBeVisible();
  }

  async function findArticleByName(scope: Page | Locator, name: string): Promise<Locator | null> {
  ...
  async function getArrangement(article: Locator): Promise<'side-by-side' | 'stacked' | 'unknown'> {
  ...
  async function getClampInfo(article: Locator) {
    return article.evaluate(el => {
      ...
      const info = (elm: Element | undefined, lines: number) => {
        ...
        const lh = parseFloat(getComputedStyle(elm as HTMLElement).lineHeight);
        return { height: rect.height, maxAllowed: lines * lh + 2, top: rect.top, left: rect.left };
      };
      return {
  ...
  test('US4-AS1: phone width (390px) — every card stacks (identity, then What/Why/Who, then footer), no horizontal scroll, same clamps', async ({
    ...
    const leadsRow = card!.getByText('LEADS', { exact: true });
  ...
  test('US4-AS2: arrangement follows the width available to the card, not the viewport — feed vs. detail dialog at one viewport', async ({
    ...
    const dialog = page.locator('[role="dialog"]').filter({ hasText: POST_TITLE }).first();
