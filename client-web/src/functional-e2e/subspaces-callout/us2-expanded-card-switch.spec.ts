...
async function createSubspace(spaceId: string, label: string, displayName: string): Promise<string> {
  const data = await rawGql<{ createSubspace: { id: string } }>(
    ...
    }`,
    {
      subspaceData: {
        ...
        about: { profileData: { displayName, description: `${displayName} — fixture subspace for US2.` } },
...
async function signIn(page: Page) {
  ...
  const switchToNewDesign = page.getByRole('button', { name: /take me to the new design/i });
...
async function openAddPostDialog(page: Page) {
  ...
  await dialog.getByRole('textbox', { name: 'Title' }).waitFor({ state: 'visible', timeout: 10_000 });
  return dialog;
}
...
async function openEditDialog(page: Page, headingText: string) {
  ...
  const dialog = page.getByRole('dialog');
  await dialog.getByRole('switch').first().waitFor({ state: 'visible', timeout: 10_000 });
  return dialog;
}

test.describe('Turn "Expanded card" on or off in the post form (US2)', { tag: '@forge-acceptance' }, () => {
    ...
    const space = await rawGql<{
      ...
      }`,
      {
        spaceData: {
          accountID,
          nameID: spaceNameId,
          about: { profileData: { displayName: `US2 Expanded Card Switch ${runSuffix}` } },
          collaborationData: { calloutsSetData: {} },
          settings: { privacy: { mode: 'PUBLIC' } },
        },
      },
  ...
  test('US2-AS1: the Expanded card switch sits after the whole Manual selection block, off by default, with the verbatim description', async ({
    ...
    const manualSwitch = dialog.getByRole('switch', { name: 'Manual selection' });
    const expandedSwitch = dialog.getByRole('switch', { name: 'Expanded card' });
    ...
    const manualBox = await dialog.getByText('Manual selection', { exact: true }).boundingBox();
    const expandedBox = await dialog.getByText('Expanded card', { exact: true }).boundingBox();
    ...
    await expect(dialog).toContainText('Shows only the subspaces you select below.');
    const manualBoxOn = await dialog.getByText('Manual selection', { exact: true }).boundingBox();
    const expandedBoxAfter = await dialog.getByText('Expanded card', { exact: true }).boundingBox();
  ...
  test('US2-AS2: publishing with the switch on renders expanded cards for admin and for an anonymous viewer', async ({
    ...
    await anonPage.goto(`${baseUrl}/${fixture.spaceNameId}`, { waitUntil: 'networkidle' });
    await anonPage.getByText(title, { exact: true }).waitFor({ state: 'visible', timeout: 15_000 });
    await expect(anonPage.getByText('What', { exact: true }).first()).toBeVisible();
  ...
  test('US2-AS3: editing an existing post shows the saved switch state and flips both directions without a full page reload', async ({
    ...
    await page.evaluate(() => {
      (window as unknown as Record<string, string>).__US2_NO_RELOAD__ = 'still-here';
    });
    ...
    expect(await page.evaluate(() => (window as unknown as Record<string, string>).__US2_NO_RELOAD__)).toBe(
      'still-here'
    );
    let settings = await readCalloutSettings(calloutId);
    expect(settings.lookup.callout.settings.framing.spaces?.cardVariant).toBe('COMPACT');

    dialog = await openEditDialog(page, title);
    await expect(dialog.getByRole('switch', { name: 'Expanded card' })).not.toBeChecked();
    await dialog.getByRole('switch', { name: 'Expanded card' }).click(); // -> on
    await page.getByRole('button', { name: /^Save$/i }).click();
    await expect(page.getByRole('dialog')).toHaveCount(0, { timeout: 15_000 });

    settings = await readCalloutSettings(calloutId);
    expect(settings.lookup.callout.settings.framing.spaces?.cardVariant).toBe('EXPANDED');
    expect(await page.evaluate(() => (window as unknown as Record<string, string>).__US2_NO_RELOAD__)).toBe(
      'still-here'
    );
  });
  ...
  test('US2-AS4: flipping Expanded card never touches a curated Manual selection (selection.mode/selectedIds survive on an API re-read)', async ({
    ...
    const calloutId = await createSubspacesPost(title, {
      framing: {
        selection: { mode: 'CUSTOM', selectedIds: [fixture.alphaId, fixture.betaId] },
    ...
    await expect(dialog.getByRole('switch', { name: 'Manual selection' })).toBeChecked();
    await expect(dialog.getByRole('switch', { name: 'Expanded card' })).not.toBeChecked();

    await dialog.getByRole('switch', { name: 'Expanded card' }).click(); // flip ONLY Expanded card
    await expect(dialog.getByRole('switch', { name: 'Manual selection' })).toBeChecked(); // untouched
    await page.getByRole('button', { name: /^Save$/i }).click();
    await expect(page.getByRole('dialog')).toHaveCount(0, { timeout: 15_000 });

    const settings = await readCalloutSettings(calloutId);
    expect(settings.lookup.callout.settings.framing.spaces?.cardVariant).toBe('EXPANDED');
    expect(settings.lookup.callout.settings.framing.selection.mode).toBe('CUSTOM');
    expect(new Set(settings.lookup.callout.settings.framing.selection.selectedIds)).toEqual(
      new Set([fixture.alphaId, fixture.betaId])
    );
    ...
    await expect(page.getByRole('link', { name: 'Beta' }).first()).toBeVisible();
  });
  ...
  test('US2-AS6: a legacy-shaped Subspaces post (stripped `framing.spaces`) reads as compact without error, and fetches no `who`/`description`', async ({
    ...
    const responsePromise = page.waitForResponse(
      res =>
        ...
        (res.request().postData() || '').includes('SpaceCollectionSubspaces') &&
        JSON.parse(res.request().postData() || '{}').variables?.calloutId === calloutId,
      { timeout: 20_000 }
    );
    await gotoFixtureSpace(page);
