...
// Callout Template with Additional Content: Memo, Collection: Memos, Comments: Disabled
...
import {
  CalloutTemplateForm,
  createAdditionalContentMemo,
  createCollectionMemos,
...
const { test, setupAuthentication, teardownAuthentication } =
  createAuthenticatedSessionFixture({
    storageStateName: 'callout-memo-template-test.json',
...
const templateData: CalloutTemplateForm = {
  // Template metadata
  displayName: 'Collaborative Documentation',
  description: 'A callout template for collaborative document creation with memo responses.',
  tags: ['callout', 'documentation'],

  // Callout base fields
  calloutTitle: 'Project Documentation',
  calloutTags: ['docs', 'collaborative'],
  calloutDescription: 'Contribute to our shared documentation. Add your memos below.',

  // Additional content: Memo
  additionalContent: createAdditionalContentMemo(
    '# Getting Started\n\nThis is the main document. Add your contributions as memos.'
  ),

  // Response options
  commentsEnabled: false,
  responseOptions: createCollectionMemos({
    defaultTitle: 'Documentation Section',
    defaultDescription: '## Section Title\n\nAdd your documentation here...',
    membersCanAdd: true,
    adminsCanAdd: true,
  }),
};

test.describe.serial('Callout Templates - Memo Additional Content', () => {
  ...
  test('1.1 Create Callout Template with Memo', async ({ page }) => {
  ...
  test('1.2 Edit Callout Template with Memo', async ({ page }) => {
    ...
    // Update memo content
    if (templateData.additionalContent.type === 'memo') {
      templateData.additionalContent.memoContent += `\n\n${editSuffix}`;
    }
  ...
  test('1.3 Delete Callout Template with Memo', async ({ page }) => {
    // Navigate to Settings > Templates
    await page.getByRole('tab', { name: 'Settings' }).click();
    await page.getByRole('tab', { name: 'Templates' }).click();
