...
| ------------ | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| US1-AS1..AS4 | `us1-default-rendering.spec.ts` | Per-tab default widget set (FR-009) renders in order for a plain member across the four canonical tab positions, including the generic `[Intention&Leads, Post Index]` default for a 4th+/custom tab.                 |
...
| US2-AS6      | `us2-admin-config.spec.ts`      | Concurrent sidebar-only and rename-only saves on the same state both persist independently.                                                                                                                           |
