...
- **Subspace Visibility**: Subspaces of private spaces are only visible to users **after they have joined the parent space**. Once a user is a member of a space, they can find subspaces under the `Subspaces` section in the subheader navigation on the space page.
...
- Level 1 (Subspace) and Level 2 (Subsubspace) are NOT visible to non-members (subspaces only visible after joining parent space)
- Level 0 shows "Apply" or "Join" button available
- Non-member cannot directly access space content beyond the landing page
- Non-member cannot see subspaces until they become a member of the parent space

#### 1.2 Verify Level 0 Member Can See Level 1 Subspaces
...
- Subspace (Level 1) appears nested under Space in the Subspaces section (visible because user is a member of Level 0)
- Subsubspace (Level 2) is NOT visible (user must be a member of Level 1 to see Level 2)
- Tree structure clearly indicates hierarchy through breadcrumbs and URL path

#### 1.3 Verify Level 1 Member Can See Level 2 Subspaces with Lock Icon

**Steps:**

1. Sign in as `${TestUser.SUBSPACE_MEMBER}@alkem.io` (a user who is a member of Level 1)
2. Navigate to Level 0 Space
3. Navigate to Level 1 Subspace
4. Verify Level 2 subsubspace is visible with lock icon
5. Click into Level 2 subsubspace and verify Apply button is present

**Expected Results:**

- Subsubspace (Level 2) appears in the Subspaces section under Level 1 (visible because user is a member of Level 1)
...
#### 1.4 Verify Level 2 Member Can Access Level 2 Subsubspace

**Steps:**

1. Sign in as `${TestUser.SUBSUBSPACE_MEMBER}@alkem.io` (a user who is a member of Level 2)
...
3a. **Test Implementation Optimization**: When implementing automated tests, sign in users once in `test.beforeAll()` or `test.beforeEach()` hooks instead of in each individual test to improve performance and reduce code duplication. Store the authenticated page/context for reuse across tests within the same describe block.
...
9. **Using baseScenario for Navigation**: The `baseScenario` variable (of type `OrganizationWithSpaceModel`) contains the space tree created for the test run. When implementing automated tests, use this structure to navigate to specific spaces programmatically:
   ...
   // Find subspaces in the 'Subspaces' section of the subheader navigation
   ...
   // Similarly, subsubspaces are only visible after joining the parent subspace
