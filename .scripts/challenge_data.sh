...
timeout 300 bash -c 'while [[ "$(curl -s -o /dev/null -w ''%{http_code}'' http://localhost:3000/api/public/rest)" != "200" ]]; do sleep 2; done' || false
echo first await
...
# Run test
npm run-script test:orphaned ./test/non-functional/orphaned-data/challenge/challenge-delete.it-spec.ts
...
diff file1.txt file2.txt | grep ">" | sed 's/^> //g' > diff_file_challenge
