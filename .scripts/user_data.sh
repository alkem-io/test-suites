...
cd $PROJECT_ROOT_DIR
pnpm run-script test:orphaned ./test/non-functional/orphaned-data/query.it-spec.ts
...
# Run test
cd $PROJECT_ROOT_DIR
pnpm run-script test:orphaned ./test/non-functional/orphaned-data/user/user-delete.it-spec.ts
