...
cd $PROJECT_ROOT_DIR
npm run-script test:orphaned ./test/non-functional/orphaned-data/user/before-all.it-spec.ts
...
# Store data before test execution
echo "$dataBefore" > file1.txt
cat file1.txt

# Run test
npm run-script test:orphaned ./test/non-functional/orphaned-data/user/user-delete.it-spec.ts
...
# Store data after test execution
...
# Compare stored data and list the tables with more data
diff file1.txt file2.txt | grep ">" | sed 's/^> //g' > diff_file_user

# Stop Alkemio service
