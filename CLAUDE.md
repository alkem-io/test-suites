...
Playwright E2E tests organized by feature area: `account-deletion/`, `applications/`, `authentication/`, `callouts/`, `chat-avatars/`, `contributors-callout/`, `default-template/`, `explore-platform/`, `identity-flows/`, `language-offer/`, `memberships/`, `messaging-notifications/`, `my-dashboard/`, `organization-space-invitations/`, `organization-user-associates/`, `public-space/`, `sidebar-search/`, `space/`, `subspaces-callout/`, `support-navigation/`, `templates/`, `templates-CRD/`, `user-profile/`. Shared code lives in `fixtures/` and `helpers/`.
