...
- TypeScript ~5.7.3, Node.js 20.9.0 (Volta) + Playwright Test (Chrome branded channel), `@alkemio/tests-lib` (MailSlurper helpers: `getVerificationLink`, `getRecoveryCode`, `getRecoveryLink`, `deleteMailSlurperMails`, `UniqueIDGenerator`, `delay`) (005-crd-auth-ui-tests)
- N/A — auth storage-state JSON persisted under `client-web/.auth/` by the session fixture (005-crd-auth-ui-tests)
